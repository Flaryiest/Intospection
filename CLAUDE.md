...
```bash
npm run sync:notion   # Sync artifacts from Notion DB to web/src/data/artifacts.json
```

Requires `NOTION_API_KEY` and `NOTION_DATABASE_ID` env vars (see `scripts/.env.example`).

## Project Structure

Monorepo with three independent workspaces:
- `web/` — React SPA (frontend)
- `api/` — Node.js backend (Express + Prisma)
- `scripts/` — Data sync tooling (Notion → JSON)
...
- `POST /api/mailing-list` — saves email to DB (validates email is present, returns 201)
- `GET /api/health` — returns status and timestamp

**Database:** PostgreSQL via Prisma. Schema defines a `MailingList` model (id, email unique, createdAt).

**Web entry flow:** `index.html` → `main.tsx` (mounts `<RouterProvider>`) → `Router.tsx` (defines routes) → page components.

**Web routing:** All routes (`/`, `/artifacts`, `/experiences`, `/writing`) nest under `Layout.tsx`, which renders `<Navbar>`, `<Outlet>`, and `<Footer>`. The `Experiences` and `Writing` pages are empty placeholder stubs. The frontend is currently static with no backend API integration.

**Artifacts data pipeline:** `scripts/sync-notion.ts` fetches from a Notion database and writes `web/src/data/artifacts.json`. The Artifacts page loads this JSON, supports tag filtering via `TagFilter` component, and renders each entry with `ArtifactItem`.
